...
namespace TengDa.Wpf
{
    ...
    public class ExcelHelper : IDisposable
    {
        ...
        public static void Export(DataTable dt, string dataName, DateTime startTime, DateTime stopTime)
        {
            Export(dt, string.Format("{0}({1}-{2})", dataName, startTime.ToString("yyyyMMddHHmmss"), stopTime.ToString("yyyyMMddHHmmss")), dataName);
        }
        /// <summary>
        /// 导出Excel方法
        /// </summary>
            ...
            if (kk.ShowDialog() == true)
            {
                ...
                try
                {
                    using (ExcelHelper excelHelper = new ExcelHelper(FileName))
                    {
                        ...
                        if (count > 0) Tip.Alert("导出为Excel成功！");
                    }
                }
                catch (Exception ex)
                {
