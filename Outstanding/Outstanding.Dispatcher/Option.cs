...
namespace Outstanding.Dispatcher
{
    public class Option
    {
        ...
        private string rotaterOpenDoorStr = string.Empty;
        /// <summary>
        /// 旋转台开门指令
        /// </summary>
        [Description("旋转台开门指令")]
        [DisplayName("旋转台开门指令")]
        [Category("旋转台")]
        public string RotaterOpenDoorStr
        {
            get
            {
                if (string.IsNullOrEmpty(rotaterOpenDoorStr))
                {
                    rotaterOpenDoorStr = TengDa.WF.Option.GetOption("RotaterOpenDoorStr");
                }
                return rotaterOpenDoorStr;
            }
            set
            {
                if (value != rotaterOpenDoorStr)
                {
                    TengDa.WF.Option.SetOption("RotaterOpenDoorStr", value);
                    rotaterOpenDoorStr = value;
                }
            }
        }

        private string rotaterCloseDoorStr = string.Empty;
        /// <summary>
        /// 旋转台关门指令
        /// </summary>
        [Description("旋转台关门指令")]
        [DisplayName("旋转台关门指令")]
        [Category("旋转台")]
        public string RotaterCloseDoorStr
        {
            get
            {
                if (string.IsNullOrEmpty(rotaterCloseDoorStr))
                {
                    rotaterCloseDoorStr = TengDa.WF.Option.GetOption("RotaterCloseDoorStr");
                }
                return rotaterCloseDoorStr;
            }
            set
            {
                if (value != rotaterCloseDoorStr)
                {
                    TengDa.WF.Option.SetOption("RotaterCloseDoorStr", value);
                    rotaterCloseDoorStr = value;
                }
            }
        }

        private string rotaterRotateStrs = string.Empty;
        /// <summary>
        /// 旋转台旋转指令
        /// </summary>
        [Description("旋转台旋转指令")]
        [DisplayName("旋转台旋转指令")]
        [Category("旋转台")]
        public string RotaterRotateStrs
        {
            get
            {
                if (string.IsNullOrEmpty(rotaterRotateStrs))
                {
                    rotaterRotateStrs = TengDa.WF.Option.GetOption("RotaterRotateStrs");
                }
                return rotaterRotateStrs;
            }
            set
            {
                if (value != rotaterRotateStrs)
                {
                    TengDa.WF.Option.SetOption("RotaterRotateStrs", value);
                    rotaterRotateStrs = value;
                }
            }
        }
